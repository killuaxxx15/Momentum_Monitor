...
st.set_page_config(page_title='ETF Model EM', page_icon=':bar_chart:')

# Display header for the dashboard
st.header('ETF Model EM')

# Display the last update date
st.markdown('#### Updated: 30/08/2024')

# Define Excel file and sheet name variables
excel_file = 'ETF_Model_EM_30_08_2024.xlsx'
...
# TABLE 1
df1 = load_excel_data(excel_file, sheet_name1, 'B:N', 5, 21)
df1 = df1.drop(['Unnamed: 6'], axis=1)
df1 = df1.rename(columns={'Unnamed: 1' : 'TICKER'})
df1 = df1.rename(columns={'Unnamed: 2' : 'ETF'})
df1 = df1.rename(columns={'Unnamed: 3' : 'COM - VALUE'})
df1 = df1.rename(columns={'Unnamed: 4' : 'COM - QUALITY'})
df1 = df1.rename(columns={'Unnamed: 5' : 'COM - SAFETY'})
df1 = df1.rename(columns={'Unnamed: 7' : 'COMPOSITE VALUE SCORE'})
df1 = df1.rename(columns={'Unnamed: 8' : 'LIQUIDITY'})
df1 = df1.rename(columns={'Unnamed: 9' : 'MOM - SCORE'})
df1 = df1.rename(columns={'Unnamed: 10' : 'MOM - UPGRADES'})
...
# TABLE 2
df2 = load_excel_data(excel_file, sheet_name2, 'C:L', 2, 21)
...
df2 = df2.rename(columns={'SAFETY' : 'COM - SAFETY'})
